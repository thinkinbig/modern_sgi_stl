...
namespace mstl {
    ...
    protected:
        ...
        void deallocate() {
            if (kStart) data_allocator::deallocate(kStart, kEndOfStorage - kStart);
        }
    ...
    public:
        // 迭代器相关
        iterator begin() { return kStart; }
        const_iterator begin() const { return kStart; }
        iterator end() { return kFinish; }
        const_iterator end() const { return kFinish; }
        
        // 容量相关
        size_type size() const { return size_type(kFinish - kStart); }
        size_type capacity() const {
            return size_type(kEndOfStorage - kStart);
        }
        bool empty() const { return kStart == kFinish; }
        ...
        Vector(size_type n, const T& value) { fillInitialize(n, value); }
        Vector(int n, const T& value) { fillInitialize(n, value); }
        Vector(long n, const T& value) { fillInitialize(n, value); }
