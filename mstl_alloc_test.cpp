...
void testMultithreadPerformance(size_t numOps, size_t size, size_t numThreads) {
    ...
    cout << "平均每次操作耗时: " << static_cast<double>(duration.count()) / numOps << " 微秒"
         << endl;
...
void testThreadContention() {
    ...
    // 测试单线程版本
    {
        ...
        std::cout << "平均每次分配时间: " << (duration.count() / (numThreads * numAllocations))
                  << "微秒\n\n";
    }
...
int main() {
    ...
    // 测试单线程性能
    cout << "\n单线程测试:" << endl;
    auto time = measure<>::execution([&]() { testAllocDealloc(kNumOperations, kBlockSize); });
    ...
    for (size_t size : blockSizes) {
        cout << "\n内存块大小 " << size << " 字节:" << endl;
        auto time = measure<>::execution([&]() { testAllocDealloc(kNumOperations, size); });
        cout << "总耗时: " << time << " 微秒" << endl;
        cout << "平均每次操作耗时: " << static_cast<double>(time) / kNumOperations << " 微秒"
             << endl;
